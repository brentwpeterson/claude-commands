# Tuesday Routine

**Day Focus:** Tuesdays with Claude publishes, update LinkedIn post with live link, rock work

---

## Date Conditionals

```
IF: P0 event within 7 days → Add "Event prep: [event name]" to punch list
```

No recurring Tuesday conditionals currently.

---

## Article Deadline Checks (MANDATORY - ask FIRST before anything else)

These are non-negotiable. Ask Brent before moving to the punch list.

| # | Article | Publishes | Question |
|---|---------|-----------|----------|
| 1 | **TWC** (Tuesdays with Claude) | Today (Tuesday) | "Is the TWC article live/published? Need the Medium link." |
| 2 | **ACG** (Agentic Commerce Guy) | Tomorrow (Wednesday) | "Is the ACG article done for tomorrow?" |

If TWC is not published yet, that's a fire. If ACG is not done, it becomes the top priority after TWC is confirmed live.

---

## Punch List

| # | Task | Tool | Time | Pull data? |
|---|------|------|------|------------|
| 1 | Clear inbox | Gmail MCP | 15 min | On demand |
| 2 | Clear HubSpot tasks | HubSpot MCP | 20 min | On demand |
| 3 | Review HubSpot deals | HubSpot MCP | 15 min | On demand |
| 4 | Review CC site | contentcucumber.com | 10 min | Manual |
| 5 | Draft Evrig daily email | Gmail MCP | 10 min | On demand |

**1 HOUR MAX. Then move to content/rocks.**

---

## Content Tasks

### 1. Tuesdays with Claude Publishes Today

The article should already be scheduled and published automatically on Medium.

**Morning check:**
- Go to Medium and get the LIVE article link
- The LinkedIn post was scheduled yesterday (Monday) but needs the link updated

### 2. Update LinkedIn Post with Medium Link

The LinkedIn post scheduled for 9 AM may have a placeholder link. Update it with the real Medium URL.

1. Log into Vista Social
2. Find today's scheduled LinkedIn post
3. Update the article link to the live Medium URL
4. Verify it posted correctly after 9 AM

### 3. Monitor Engagement

Throughout the day:
- Check Medium stats (reads, claps, highlights)
- Check LinkedIn post engagement (likes, comments, shares)
- Respond to any comments

---

## Rock Work

With content handled, focus on Q1 Rocks:

1. Review ROCKS.md for current priorities
2. Identify one rock to make progress on today
3. Block 2-3 hours for deep work

---

## Routine Feedback

**After completing this routine, Claude should ask:**

> "Tuesday routine complete. Anything to add or remove from this routine for future Tuesdays?"

If user suggests changes, update this file.
